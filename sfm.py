...
def epipolar_distance(a, b, E):
    ...
    # Calculate the epipolar distance
    d_B = np.abs(b.T @ l / np.linalg.norm(l[:2]))
    d_A = np.abs(a.T @ r / np.linalg.norm(r[:2]))
...
def getE(a, b, K, rng, threshold=1e-3, num_iters=1000):
    ...
    """
    a_hom = np.column_stack([a, np.ones(a.shape[0])])
    b_hom = np.column_stack([b, np.ones(b.shape[0])])

    alpha = []
    beta = []
    for i in range(len(a_hom)):
        alpha.append(np.linalg.inv(K) @ a_hom[i])
        beta.append(np.linalg.inv(K) @ b_hom[i])
    ...
    for i in range(num_iters):
        ...
        idx = rng.choice(len(alpha), 8, replace=False)
        alpha_sample = alpha[idx]
        beta_sample = beta[idx]
        constraint_matrix = []
        for j in range(len(alpha_sample)):
            constraint_matrix.append(np.kron(alpha_sample[j], beta_sample[j]).T)
        constraint_matrix = np.array(constraint_matrix)
        _, _, Vt = np.linalg.svd(constraint_matrix)
        E = Vt[-1].reshape(3, 3, order='F')
        E = (np.sqrt(2) / np.linalg.norm(E))*E
        
        # Correct essential matrix
        U_prime, _, Vt_prime = np.linalg.svd(E)
        V_prime = Vt_prime.T
        S_corrected = np.diag([1., 1., 0.])
        U_corrected = np.column_stack([U_prime[:, 0], U_prime[:, 1], np.linalg.det(U_prime) * U_prime[:, 2]])
        V_corrected = np.column_stack([V_prime[:, 0], V_prime[:, 1], np.linalg.det(V_prime) * V_prime[:, 2]])
    
        E = U_corrected @ S_corrected @ V_corrected.T
        #Get the number of inliers using the epipolar distance and the threshold epsilon
        num_inliers = 0
        mask = []
        for k in range(len(alpha)):
            d_A_i, d_B_i = epipolar_distance(alpha[k], beta[k], E)
            if d_A_i < threshold and d_B_i < threshold:
                num_inliers += 1
                mask.append(True)
            else:
                mask.append(False)
        #Update the best estimate of the essential matrix if the current estimate has more inliers
        if num_inliers > best_num_inliers:
            best_E = E
            best_num_inliers = num_inliers
            best_mask = mask

    return best_E, best_num_inliers, best_mask

def twoview_triangulate(alpha, beta, R_inB_ofA, p_inB_ofA):
    p_inA = []
    p_inB = []
    for i in range(len(alpha)):
        row1 = -skew(beta[i]) @ R_inB_ofA
        row2 = skew(alpha[i])
        A = np.row_stack([row1, row2])
        b = np.concatenate((skew(beta[i]) @ p_inB_ofA, np.zeros(3)), axis=0)
        p_inA_i = np.linalg.pinv(A) @ b
        p_inA.append(p_inA_i)
        p_inB.append(R_inB_ofA @ p_inA_i + p_inB_ofA)
    p_inA = np.array(p_inA)
    p_inB = np.array(p_inB)
    # Get mask
    mask = []
    for i in range(len(p_inA)):
        mask.append(p_inA[i,2] > 0 and p_inB[i,2] > 0)
    mask = np.array(mask)
...
def decomposeE(a, b, K, E):
    ...
    R1 = U_corrected @ W.T @ V_corrected.T
    p1 = U_corrected[:, 2]
    p_inA_1, p_inB_1, mask_1 = twoview_triangulate(alpha, beta, R1, p1)

    R2 = U_corrected @ W @ V_corrected.T
    p2 = -U_corrected[:, 2]
    p_inA_2, p_inB_2, mask_2 = twoview_triangulate(alpha, beta, R2, p2)

    R3 = U_corrected @ W.T @ V_corrected.T
    p3 = -U_corrected[:, 2]
    p_inA_3, p_inB_3, mask_3 = twoview_triangulate(alpha, beta, R3, p3)

    R4 = U_corrected @ W @ V_corrected.T
    p4 = U_corrected[:, 2]
    p_inA_4, p_inB_4, mask_4 = twoview_triangulate(alpha, beta, R4, p4)
  
    # Count number of non-zero elements in each mask and choose most consistent solution
    masks_nonzero_counts = [np.count_nonzero(mask_1), np.count_nonzero(mask_2), np.count_nonzero(mask_3), np.count_nonzero(mask_4)]
    most_consistent = np.argmax(masks_nonzero_counts)
    if most_consistent == 0:
        print('Candidate 1 is the most consistent')
        return R1, p1, p_inA_1
    elif most_consistent == 1:
        print('Candidate 2 is the most consistent')
        return R2, p2, p_inA_2
    elif most_consistent == 2:
        print('Candidate 3 is the most consistent')
        return R3, p3, p_inA_3
    elif most_consistent == 3:
        print('Candidate 4 is the most consistent')
        return R4, p4, p_inA_4
...
def resection(p_inA, c, K, rng, threshold=1., num_iters=1000):
    ...
    c_hom = np.column_stack([c, np.ones(c.shape[0])])
    gamma = []
    for i in range(len(c_hom)):
        gamma.append(np.linalg.inv(K) @ c_hom[i])
    ...
    for i in range(num_iters):
        constraint_mat = []
        idx = rng.choice(len(p_inA), 6, replace=False)
        p_inA_sample = p_inA[idx]
        gamma_sample = gamma[idx]
        for j in range(len(p_inA_sample)):
            block_elem = np.column_stack([np.kron(p_inA_sample[j], skew(gamma_sample[j])), skew(gamma_sample[j])])
            constraint_mat.append(block_elem[0])
            constraint_mat.append(block_elem[1])
            constraint_mat.append(block_elem[2])

        constraint_mat = np.array(constraint_mat)
        U, S, Vt = np.linalg.svd(constraint_mat)
        sol1 = Vt[-1]/np.linalg.norm(Vt[-1,:3])
        rot_matrix_1 = np.column_stack([sol1[:3], sol1[3:6], sol1[6:9]])
        sol2 = sol1*np.linalg.det(rot_matrix_1)
        U, S, Vt = np.linalg.svd(sol2[:9].reshape(3,3,order='F'))
        R_inC_ofA = np.linalg.det(U @ Vt) * U @ Vt
        p_inC_ofA = sol2[9:12]

        num_inliers = 0
        mask = []
        for k in range(len(p_inA)):
            if projection_error(K, R_inC_ofA, p_inC_ofA, p_inA[k], c[k]) < threshold:
                # print('Inlier')
                num_inliers += 1
                mask.append(True)
            else:
                mask.append(False)
        if num_inliers > best_num_inliers:
            best_R_inC_ofA = R_inC_ofA
            best_p_inC_ofA = p_inC_ofA
            best_num_inliers = num_inliers
            best_mask = mask
    
    return best_R_inC_ofA, best_p_inC_ofA, best_num_inliers, best_mask
...
def triangulate(track, views, K):
    ...
    for match in track['matches']:
        ...
        pt2d = view['pts'][feature_id]['pt2d']
        beta = np.linalg.inv(K) @ np.array([pt2d[0], pt2d[1], 1])
        block_elem_mat1 = skew(beta) @ R_inB_ofA
        block_elem_mat2 = -skew(beta) @ p_inB_ofA
        mat1.append(block_elem_mat1[0])
        mat1.append(block_elem_mat1[1])
        mat1.append(block_elem_mat1[2])
        mat2.append(block_elem_mat2[0])
        mat2.append(block_elem_mat2[1])
        mat2.append(block_elem_mat2[2])
    
    mat1 = np.array(mat1)
    mat2 = np.array(mat2)

    p_inA = np.linalg.pinv(mat1) @ mat2 #Solving for p_inA using the equation : mat1 * p_inA = mat2
